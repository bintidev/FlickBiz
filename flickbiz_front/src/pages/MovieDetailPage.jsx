...
import {
  FiArrowLeft, FiHeart, FiClock, FiEye, FiCheck, FiStar, FiMessageSquare
} from "react-icons/fi";
import { getMovieDetail, toggleFavorite, removeFavorite, setStatus, removeStatus } from "../services/mediaService";
import { getReviews } from "../services/reviewService";
import toast from "react-hot-toast";

const ST = [
  { value: "watchlist", label: "Watchlist", icon: FiClock,  color: "#a78bfa", bgAlpha: "rgba(167,139,250,0.03)", bgHover: "rgba(167,139,250,0.15)", glow: "rgba(167,139,250,0.4)" },
  { value: "watching",  label: "Watching",  icon: FiEye,    color: "#ff8c42", bgAlpha: "rgba(255,140,66,0.03)",  bgHover: "rgba(255,140,66,0.15)",  glow: "rgba(255,140,66,0.4)" },
  { value: "watched",   label: "Watched",   icon: FiCheck,  color: "#34d399", bgAlpha: "rgba(52,211,153,0.03)",  bgHover: "rgba(52,211,153,0.15)",  glow: "rgba(52,211,153,0.4)" },
];
...
export default function MovieDetailPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [mRes, rRes] = await Promise.all([getMovieDetail(id), getReviews({ movie: id })]);
        setMovie(mRes.data);
  ...
  const handleSt = async (s) => {
    try {
      if (st === s) {
        await removeStatus({ movie: id });
        setSt(null);
      } else {
        await setStatus({ movie: id, status: s });
        setSt(s);
      }
  ...
  if (!movie) return <div className="min-h-screen flex items-center justify-center bg-[#05050a] text-[#8888aa]">Movie not found.</div>;

  const year = movie.release_date ? new Date(movie.release_date).getFullYear() : "—";
  const providers = [...new Set((movie.availability || []).map(a => a.provider).filter(Boolean))];

  return (
    <div className="w-full min-h-screen lg:h-screen bg-[#05050a] flex flex-col lg:flex-row antialiased lg:overflow-hidden select-none relative">
      
      {/* Luz ambiental estática de fondo - Optimizada para no romper en mobile */}
      ...
      <section className="w-full lg:w-[45%] h-[55vh] sm:h-[65vh] lg:h-full relative lg:absolute lg:inset-y-0 lg:left-0 overflow-hidden border-b lg:border-b-0 lg:border-r border-white/10 z-20 flex-shrink-0">
        ...
        >
          <img src={movie.poster || movie.backdrop} alt="" className="w-full h-full object-cover object-top lg:object-center" />
        </motion.div>

        {/* Degradado adaptativo: más oscuro en móvil en la base para fusionarse con el fondo general */}
        ...
        {/* Contenedor de información del título */}
        <div className="absolute bottom-0 inset-x-0 p-4 md:p-8 z-20 pointer-events-auto bg-gradient-to-t from-[#05050a] via-[#05050a]/80 to-transparent lg:bg-none lg:p-6 lg:bottom-6 lg:left-6 lg:right-6">
          ...
          <div className="flex items-center gap-2 lg:gap-2.5 text-[10px] lg:text-[11px] mt-2 lg:mt-3 text-white/60 font-mono bg-black/50 backdrop-blur-md px-2.5 py-1 rounded-lg w-fit border border-white/5 shadow-lg">
            <span className="text-[#ff8c42] font-bold flex items-center gap-0.5"><FiStar size={11} fill="#ff8c42" /> {movie.average_rating?.toFixed(1) || "—"}</span>
            <span className="text-white/20">•</span><span>{year}</span>
            <span className="text-white/20">•</span><span className="text-[#a78bfa] font-semibold">{movie.runtime || "—"} MIN</span>
          </div>
        </div>
      </section>

      {/* ── MITAD DERECHA: FICHA TÉCNICA DINÁMICA CON ESPACIADO RESPONSIVE ── */}
      <main className="w-full lg:w-[55%] lg:ml-[45%] h-auto lg:h-full px-4 sm:px-8 md:px-12 py-8 lg:py-14 flex flex-col gap-8 lg:gap-10 z-10 relative overflow-y-visible lg:overflow-y-auto scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
        ...
        <section className="relative pl-4 border-l-2 border-[#ff3f6c] bg-gradient-to-r from-[#ff3f6c]/3 to-transparent py-2.5 lg:py-3.5 pr-4 rounded-r-xl">
          ...
          <p className="text-xs sm:text-sm lg:text-base font-light text-white/90 leading-relaxed tracking-tight">
            {showFullSynopsis ? movie.synopsis : (movie.synopsis?.slice(0, 180) + (movie.synopsis?.length > 180 ? "…" : ""))}
          </p>
        ...
        {/* Ficha Técnica (De 1 columna en mobile a 3 columnas en desktop) */}
        <section className="grid grid-cols-1 sm:grid-cols-3 gap-3.5 sm:gap-4 border-t border-b border-white/5 py-5 bg-gradient-to-r from-white/[0.01] to-transparent px-3 rounded-xl">
          <div className="flex flex-col gap-1.5">
            <span className="text-[9px] font-mono text-white/30 uppercase tracking-wider">Director</span>
            <span className="text-white font-bold text-[11px] bg-white/[0.02] px-3 py-2 rounded-lg border border-white/5 block text-center sm:text-left" style={{ fontFamily: "var(--font-display)" }}>
              {movie.director || "Unknown"}
            </span>
          </div>

          <div className="flex flex-col gap-1.5">
            <span className="text-[9px] font-mono text-white/30 uppercase tracking-wider">Language Track</span>
